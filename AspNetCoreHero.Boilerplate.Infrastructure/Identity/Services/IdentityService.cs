...
namespace AspNetCoreHero.Boilerplate.Infrastructure.Identity.Services
{
    ...
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<Result<string>> RegisterAsync(AutoRegisterRequest request, string origin)
        {
            ...
            {
                var hashPassword = request.Password ;
        ...
        public async Task<Result<string>> ConfirmEmailAsync(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                ...
                //var userData = new UserSignUpRequest { firstName=user.FirstName,lastName=user.LastName, email =user.Email,userName=user.UserName,password=user.PasswordHash512,confirmPassword=user.PasswordHash512,referredBy= "XO19S", referredByEmail="",countryCode="AE", createdAt = currentTimestampMillis };
